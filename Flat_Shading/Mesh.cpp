...
void Mesh::SetUpMesh()
{
	...
	glBufferData(GL_ARRAY_BUFFER, vertices.size() * sizeof(Vertex), &vertices[0], GL_STATIC_DRAW); //we get the pointer to the data by reffering to the mem pos of the first vertex
	...
	std::cout << "CREATED VAO WITH ID "<< VAO << std::endl;
}
