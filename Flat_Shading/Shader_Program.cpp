...
void Shader_Program::set_uniforms() {
	for (const auto& uniform : vec3_uniforms) {
		Set_Vec3(uniform.first, uniform.second);
	}
	//set vec4 uniforms
	for (const auto& uniform : vec4_uniforms) {
		Set_Vec4(uniform.first, uniform.second);
	}
	//set mat3 uniforms
	for (const auto& uniform : mat3_uniforms) {
		Set_Mat3(uniform.first, uniform.second);
	}
	//set mat4 uniforms
	for (const auto& uniform : mat4_uniforms) {
		Set_Mat4(uniform.first, uniform.second);
	}
	//set float
	for (const auto& uniform : float_uniforms) {
		Set_Float(uniform.first, uniform.second);
	}
