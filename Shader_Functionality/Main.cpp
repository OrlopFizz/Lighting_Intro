...
int main() {
	...
	*/
	...
	glm::vec3 object_color = glm::vec3(191.0f / 255.0f, 0.0f / 255.0f, 255.0f / 255.0f);
	...
	Skybox* skybox = new Skybox(texture_dict, shader_program_skybox);
...
*/
	...
	while (!glfwWindowShouldClose(window)) {
		...
		skybox->mat4_rendering_info["view"] = glm::mat4(glm::mat3(view_space)); //we get rid if the translation part of the view matrix by turning it into a mat3.
		skybox->mat4_rendering_info["projection"] = projection_space;
		skybox->draw();

		//NORMAL SHADER UNIFORMS
		teacup->set_camera_info(*camera, win_width, win_height);
		teacup->set_material_info();
		teacup->set_light_info(*light_1);
		teacup->draw();

		//PLANE SHADER UNIFORMS
		//TODO plane shaders do not recieve all this info!
		//it only needs the ModelViewProjection matrix
		plane->set_camera_info(*camera, win_width, win_height);
