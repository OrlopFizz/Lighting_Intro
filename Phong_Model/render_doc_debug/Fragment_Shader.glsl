//FRAGMENT SHADER
#version 460
in vec3 diffuse_light_intensity;
in vec3 specular_light_intensity;
in vec3 ambient_light_intensity;
in vec2 tex_coord;
...
void main(){
	...
	FragColor = vec4(ambient_light_intensity, 1.0) + vec4(diffuse_light_intensity, 1.0); + vec4(specular_light_intensity, 1.0);
}